...
![](https://github.com/JZZ-NOTE/Algorithm/raw/main/picture/19.png)

**左旋**
逆时针旋转又称左旋，旋转的结果是 左孩子成为根，旋转的细节如下图所示：

![](https://github.com/JZZ-NOTE/Algorithm/raw/main/picture/20.png)

AVL树的每个节点自带其子树的高度信息，每添加一个节点，会回溯修改其祖先节点的高度信息。这样就可以发现不平衡。

![](https://github.com/JZZ-NOTE/Algorithm/raw/main/picture/21.png)

发现不平衡后调整的组合有4种，LL、LR、RR、RL
LL、RR 一个动作
LR、RL 两个动作

![](https://github.com/JZZ-NOTE/Algorithm/raw/main/picture/22.png)

![](https://github.com/JZZ-NOTE/Algorithm/raw/main/picture/23.png)

### LeetCode 218 天际线问题

按横坐标依次遍历节点，最大高度发生变化时，会产生轮廓
...
///定义结点，每个结点具有 位置、高度、上下三个信息
struct Node{
    bool isUp;
    int posi;
    int h;
    Node(bool _isUp,int _posi,int _h):isUp(_isUp),posi(_posi),h(_h){}
};
///自定义比较器
///按位置排序，位置小的排在前面，位置相同的，下降的排在前面，上升的排在后面
...
//    ///位置相同的怎么处理都行
//    if(o1.isUp != o2.isUp){
//        return o1.isUp ? false:true;
//    }
    return true;
}

vector<vector<int>> buildingOutline(vector<vector<int>> buildings){
    vector<Node> nodes(buildings.size()*2,Node(true,0,0));
    for(int i=0;i<buildings.size();++i){
        nodes[i*2] = Node(true,buildings[i][0],buildings[i][2]);
        nodes[i*2+1] = Node(false,buildings[i][1],buildings[i][2]);
    }
    ...
    int i=1;
    for(auto n:nodes){
        cout<<i<<":"<<n.posi<<" "<<n.h<<" "<<n.isUp<<endl;
        ++i;
    }

    map<int,int> htMap; ///高度的红黑树 key是某一个高度，value是这个高度出现的次数
    map<int,int> pmMap; ///每个位置的最大高度，如果是下降位置，位置为0
    for(int i=0;i<nodes.size();++i){
        ...
        if(htMap.empty()){
            ...
            pmMap[nodes[i].posi]=(--htMap.end())->first;
        }
    }
    ...
    for(auto entry: pmMap){
        int curPosition = entry.first;
...
求以每个数结尾的和为k的最长子数组，答案就在其中。

1、记录从0开始每一个和出现的的最早位置
2、累加和
3、假如遍历到了数字5，此时从0开始累加和为sum，要想得到一个以 5 结尾的和为 k 的子数组，就要找到从 0 开始和为 sum-k 的位置，之前记录了最开始出现和为 sum-k 的位置，便可以知道 以5结尾的和为 k 的子数组的最大长度

此题使用 哈希表 即可解，不需要用到 二叉搜索树。
```cpp
#include <vector>
#include <iostream>
#include <algorithm>
...
int main()
{
    vector<int> nums{7,3,2,1,1,7,-6,-1,7};
    cout<<maxLength(nums,7)<<endl;
    return 0;
}
```
...
与上一题思路相同，不同之处在于 map中记录之前出现的次数，而不是最早出现的位置。
...
令奇数为 -1，偶数为 1，求 和为0 的最长子数组

### 异或和为0的子数组的最大数量

**异或的性质：**
1. 异或运算满足交换律和结合律。换句话说 一些数异或的结果与异或的顺序无关
2. 0 与任何数异或都得该数本身；一个数异或自己得 0 

使用动态规划求解，以 i 结尾的情况与以 i-1 结尾的情况的关系分两种情况，
1. 如果以 i 结尾的最优划分中，i 所在的数组异或和为 0，那么 dp[i] = dp[k-1] +1。关于 k，如果从0 -i 的异或和为 sum，那么 k 为 从 0 到 某个离 i 最近的异或和也为 sum 的位置的下一个
2. 如果以 i 结尾的最优划分中，i 所在的数组异或和不为 0，那么 dp[i] = dp[i-1] 
...
int mostEOR(vector<int>& nums){
    if(nums.empty()){
        return 0;
    }
    ///dp数组
    vector<int> dp(nums.size(),0);
    int sum=0;  ///求出每个位置的累加和
    unordered_map<int,int> map; ///记录 每个sum 最新出现的位置
    map[0]=-1;
    for(int i=0;i<nums.size();++i){
        sum^=nums[i];
        if(map.find(sum)!=map.end()){
            int pre=map[sum];
            dp[i]=pre==-1? 1:dp[map[sum]]+1;
        }
        if(i>0){
...
int main()
{
    vector<int> nums{1,2,3,0,3,2,1,0};
    cout<<mostEOR(nums)<<endl;
    return 0;
}
```
...
与上题相比，多了一个正数的条件，可以有更好的方法，
**双指针**
这样可以节省空间复杂度，空间复杂度为O(1)

### 和小于等于k的最长子数组(正数、负数、0)
两个辅助数组：
min_sum：以 i 位置开头的子数组的最小累加和
min_sum_index：此时的右边界，有多个时任写一个
从最后一个位置往前写，遍历一遍就可以得到这两个数组

### 整数数组中奇偶相同的最长子数组
思路：把奇数都变为1，偶数变为-1，等同于求 累加和为0 的最长子数组
同理，一个数组中含有0、1、2，求含有 1、2 数量相同的最长子数组。把 2 变 -1，等同于求 累加和为0 的最长子数组

###  [LintCode 1844] 子数组和为k（和为k的最短子数组）

看到子数组问题就想以某个数会怎么样
与下一题最长的思路类似，只须修改：
1、每次都更新map[sum]，因为要知道 和为sum 的最新位置
2、len 由 min 得到
3、len初始化得是个大值，因为要由 min 得到
